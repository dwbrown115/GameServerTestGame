...
namespace Mechanics.Chaos
{
    ...
    public class RippleOnHitMechanic : MonoBehaviour, IMechanic
    {
        ...
        public void Tick(float dt) { /* no per-frame work on host */ }
        ...
        public void TriggerFrom(Transform initialTarget, Vector2 hitPoint)
        {
            if (_ctx == null)
                return;
            if (debugLogs)
                Debug.Log($"[RippleOnHit] Trigger at {hitPoint} initial={initialTarget?.name}", this);
        ...
        private void ContinueChainFrom(Transform newTarget, Vector2 newCenter, int remaining)
        {
            ...
            {
                if (debugLogs)
                    Debug.Log($"[RippleOnHit] Chain -> {newTarget?.name} rem={remaining - 1}", this);
                SpawnRippleChain(newCenter, newTarget, remaining - 1);
            }
            else if (debugLogs)
            {
                Debug.Log("[RippleOnHit] Chain roll failed", this);
            }
        }
        ...
        {
            [HideInInspector] public RippleOnHitMechanic parent;
            [HideInInspector] public Transform owner;
            [HideInInspector] public Transform excludeTransform;
            [HideInInspector] public int remainingChains;

            [HideInInspector] public float startRadius;
            [HideInInspector] public float endRadius;
            [HideInInspector] public float growDuration;
            [HideInInspector] public float edgeThickness;
            [HideInInspector] public int rippleDamage;
            [HideInInspector] public LayerMask targetLayers;
            [HideInInspector] public bool requireMobTag;
            [HideInInspector] public bool excludeOwner;
            [HideInInspector] public bool showVisualization;
            [HideInInspector] public Color vizColor;
            [HideInInspector] public int vizSortingOrder;
            [HideInInspector] public bool debugLogs;
            ...
            public void Initialize(MechanicContext ctx) { /* not required */ }
            ...
            private void UpdateViz()
            {
                ...
                for (int i = 0; i < seg; i++)
                {
                    ...
                    _line.SetPosition(i, new Vector3(transform.position.x + x, transform.position.y + y, 0f));
                }
            }
            ...
            private void DoOnTouchDamage()
            {
                ...
                foreach (var c in overlaps)
                {
                    ...
                    if (excludeTransform != null && (c.transform == excludeTransform || c.transform.IsChildOf(excludeTransform)))
                        continue;
                    ...
                    total += rippleDamage;
                    if (debugLogs)
                        Debug.Log($"[RippleOnHit:Instance] Damaged {c.name} for {rippleDamage}", this);
