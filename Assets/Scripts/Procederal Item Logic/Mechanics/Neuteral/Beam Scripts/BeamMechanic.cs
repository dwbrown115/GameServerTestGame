...
namespace Mechanics.Neuteral
{
    ...
    public class BeamMechanic : MonoBehaviour, IMechanic
    {
        ...
        // Cache of modifier -> declared target segment set (null means all)
        private Dictionary<Mechanics.IBeamSegmentModifier, HashSet<string>> _modifierTargets =
            new Dictionary<Mechanics.IBeamSegmentModifier, HashSet<string>>();
        ...
        private void EnsureSegmentModifiers(bool force = false)
        {
            ...
            foreach (var m in _segmentModifiers)
            {
                ...
                try
                {
                    targets = m.GetTargetSegments();
                }
                catch (System.Exception ex)
                {
                    if (debugLogs)
                        Debug.LogWarning(
                            "[BeamMechanic] GetTargetSegments exception: " + ex.Message,
                            this
                        );
                }
                if (targets == null || targets.Length == 0)
                {
                    _modifierTargets[m] = null; // null => all segments
                }
                else
                {
                    ...
                    _modifierTargets[m] = set.Count == 0 ? null : set;
                }
            }
            if (debugLogs)
                Debug.Log(
        ...
        private void NotifySegmentDamage(int headDamage, int tailDamage)
        {
            ...
            if (_segmentModifiers == null || _segmentModifiers.Length == 0)
                return;
            // Build local damage map for this tick
            // Only standard segments for now (head, tail); can be extended with additional segments later.
            if (headDamage > 0 || tailDamage > 0)
            {
                foreach (var m in _segmentModifiers)
                {
                    if (m == null)
                        continue;
                    if (!_modifierTargets.TryGetValue(m, out var targetSet))
                        targetSet = null; // treat as all
                    // Head
                    if (headDamage > 0 && (targetSet == null || targetSet.Contains("head")))
                    {
                        try
                        {
                    ...
                    // Tail
                    if (tailDamage > 0 && (targetSet == null || targetSet.Contains("tail")))
                    {
                        try
                        {
        ...
        // Reusable list for generic hit modifiers
        private static System.Collections.Generic.List<Mechanics.IPrimaryHitModifier> _primaryHitMods =
        ...
        private int DamageUniqueHits(int count, HashSet<Collider2D> seen)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (dmg == null || !dmg.IsAlive)
                    continue;
                Vector2 hitPoint = c.bounds.ClosestPoint(transform.position);
                Vector2 hitNormal = _dir; // forward dir approximation (beam directional)
                ...
                DispatchPrimaryHit(c.transform, hitPoint, hitNormal, damagePerInterval);
            }
            return total;
        }
        ...
        {
            _primaryHitMods.Clear();
            GetComponents(_primaryHitMods);
            var info = new Mechanics.PrimaryHitInfo(target, hitPoint, hitNormal, dmg, this);
            for (int i = 0; i < _primaryHitMods.Count; i++)
            {
                var mod = _primaryHitMods[i];
                if (mod == null)
                    continue;
                try
                {
                ...
                {
                    if (debugLogs)
                        Debug.LogWarning($"[BeamMechanic] Modifier exception: {ex.Message}", this);
                }
            }
        }
