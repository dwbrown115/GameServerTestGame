...
namespace Mechanics.Neuteral
{
    ...
    public static class AuraMechanicSettings
    {
        public static void Apply(AuraMechanic comp, IDictionary<string, object> s)
        {
            ...
            if (TryGet<bool>(s, "centerOnTarget", out var cot))
                comp.centerOnTarget = cot;
            if (TryGet<bool>(s, "excludePlayer", out var ep))
                comp.excludePlayer = ep;
            if (TryGet<bool>(s, "requireMobTag", out var rm))
                comp.requireMobTag = rm;
