...
public class RuntimeCollectibleValidator : MonoBehaviour
{
    ...
    {
        Collectible[] collectiblesInScene = FindObjectsByType<Collectible>(
        ...
        foreach (Collectible collectible in collectiblesInScene)
        {
            ...
            if (collectible.gameObject.activeInHierarchy)
            {
                ...
                {
                    Debug.LogWarning(
                        $"RuntimeCollectibleValidator: Duplicate object '{objectName}' found in scene during runtime check. Destroying duplicate."
                    );
                ...
                {
                    Debug.LogWarning(
                        $"RuntimeCollectibleValidator: Object '{objectName}' has an invalid number format during runtime check. Destroying object."
                    );
                    Destroy(collectible.gameObject);
                    continue; // Move to the next collectible
                }
                ...
                {
                    Debug.LogWarning(
                        $"RuntimeCollectibleValidator: Object '{objectName}' is not found in validatedObjects.json during runtime check. Destroying object."
                    );
                    Destroy(collectible.gameObject);
                    continue; // Move to the next collectible
                }
